## This code clone from https://github.com/lernapparat/lernapparat
import torch
...
class NoiseLayer(nn.Module):
    ...
    def forward(self, x, noise=None):
        if noise is None and self.noise is None:
            ...
            noise = self.noise
        self.noise = noise
        x = x + self.weight.view(1, -1, 1, 1) * self.noise
        return x
...
class LayerEpilogue(nn.Module):
    ...
    def forward(self, x, dlatents_in_slice=None):
        # x = self.top_epi[0](x, noise) # Noise Layer
        # x = self.top_epi[1:](x) # rest epilogue
        x = self.top_epi(x)
